...
class AuthController extends Controller
{
    ...
    public function attemptLogin()
    {
        ...
        $dataSession =[
            ...
            'captcha' => '<img class="rounded-circle mt-1"  width="32" height="32" src="data:image/png;base64,' . $timage->toImg64() . '" />',
            'captcha2' => '<img class="rounded-circle mt-1"  width="40" height="40" src="data:image/png;base64,' . $timage->toImg64() . '" />',
        ...
        if(!file_exists(WRITEPATH.'uploads'.$b.$this->auth->user()->username)){
            mkdir(WRITEPATH.'uploads'.$b.$this->auth->user()->username);
        }
    ...
    public function register()
    {
        ...
        return $this->_render($this->config->views['register'], ['config' => $this->config, 'captcha' => $captcha]);
    }
    ...
    public function attemptRegister()
    {
        ...
        $textCaptcha = session()->getFlashdata('captchaText');
        ...
        if($textCaptcha != $this->request->getPost('captcha')){
            return redirect()->back()->withInput()->with('error', 'Not Match Captcha');
        }
        ...
        if ($this->config->requireActivation !== null) {
            ...
            // Success!
            return redirect()->route('login')->with('message', lang('Auth.waitAdministrator'));
        }
    ...
    public function activateAccount()
    {
        ...
        if(!$this->request->getGet('view') == 'admin'){
